	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				...
				levelnol: '*Nivel* 0 ',
				error: {
					stick: '[❗] Falló, se produjo un error al convertir la imagen en una pegatina ❌ intentalo de nuevo',
					Iv: '❌ Link inválido ❌'
				},
				only: {
					...
					Badmin: '[❗] Este comando solo se puede usar cuando el bot se convierte en administrador! ❌',
                                        daftarB: `「Bakaaaa!」\n\nEspera...\n\nNo estas registrado en mi base de datos (｡ŏ﹏ŏ) \n\nComando : ${prefix}daftar Nombre\nEjemplo : ${prefix}daftar Zero`,
				}
			}
		...
		case 'spamcall':
               client.updatePresence(from, Presence.composing)
				...
				case 'ytmp3':
					...
					teks = `❏ *Título* : ${anu.title}\n❏ *Tamaño del archivo* : ${anu.result.size}\n\nNO SPAMES TE ESTOY ENVIANDO EL AUDIO ESPERAME u.u`
				...
				case 'ytmp4':
					...
					if (anu.error) return reply(anu.error)
					teks = `*❏ Título* : ${anu.title}\n\n*EL VIDEO SE ESTÁ ENVIANDO, no spam u.u*`
					thumb = await getBuffer(anu.thumb)
					client.sendMessage(from, thumb, image, {quoted: mek, caption: teks})
					buffer = await getBuffer(anu.result)
			...
			tels = body.slice(6)
               ...
               if (data.error) return reply(data.error)
				...
				client.updatePresence(from, Presence.composing) 
					...
					client.sendMessage(from, '╔══✪〘 Mencionar A Todos 〙✪══\n╠➥'+teks+'╚═〘 ZeroBot 〙', text, {detectLinks: false, quoted: mek})
					break
                        case 'tagall4':
				client.updatePresence(from, Presence.composing) 
					if (!isGroup) return reply(mess.only.group)
				...
				case 'alay':
                    ...
                    client.sendMessage(from, hasil, image, {quoted: mek, caption: 'ehh...'})
                    break
                    case 'glitch':
                    gh = body.slice(7)
                    if (!isUser) return reply(mess.only.daftarB)
          ...
          contextInfo: { mentionedJid: [from] }
           ...
           break
				...
				case 'add':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (args.length < 1) return reply('mmm okey')
					if (args[0].startsWith('08')) return reply('Soy un bot,(◕‿◕)')
					try {
				...
				case 'kick':
					...
					if (mentioned.length > 1) {
						...
						client.groupRemove(from, mentioned)
					client.sendMessage(mentioned, 'Chao Baka', text)
					}
					break
				case 'exe':
	              client.updatePresence(from, Presence.composing) 
	    ...
	    case 'kapankah':
			...
			case 'closegc':
				...
				case 'toimg':
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						...
						client.sendMessage(from, buffer, image, {quoted: mek, caption: 'ERROR, intente de nuevo :l'})
						fs.unlinkSync(ran)
					})
					break
                	case 'tomp3':
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						fs.unlinkSync(media)
						if (err) return reply('❌ No se pudo convertir el video a mp3, intente de nuevo ❌')
						buffer = fs.readFileSync(ran)
                ...
                break
                   ...
                   break
					...
					break
                                  case 'daftar':
					client.updatePresence(from, Presence.composing)
					if (isUser) return reply('Ya estas registrado (>‿◠)✌')
					...
					var umure = reg.split("|")[1];
						...
						client.sendMessage(from, `\`\`\`Registrado con exito usuario kun ❤（っ＾▿＾）\`\`\`\n\n\`\`\`DNI: secreto(>‿◠)✌\`\`\`\n\n\`\`\`Dia ${date} ${time}\`\`\`\n\`\`\`[Usuario]: ${jeneng}\`\`\`\n\`\`\`[Número]: wa.me/${sender.split("@")[0]}\`\`\`\n\`\`\`Para usar el bot\`\`\`\n\`\`\`Por favor\`\`\`\n\`\`\`enviar ${prefix}help\`\`\`\n\`\`\`\nTotal de usuários ${user.length}\`\`\``, text, {quoted: mek})
					break
                                case 'welcome':
            ...
            break
						...
						try {
							...
							buffer = await getBuffer(res.result)
							client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Otaku pervertido u-u'})
						} catch (e) {
							console.log(`Error :`, color(e,'red'))
							reply('❌ *ERROR* ❌')
						}
					...
					case 'nsfwbobs': 
						try {
							...
							buffer = await getBuffer(res.url)
							client.sendMessage(from, buffer, image, {quoted: mek, caption: '(｡>﹏<｡)'})
						} catch (e) {
							console.log(`Error :`, color(e,'red'))
							reply('❌ *ERROR* ❌')
						}
				...
				case 'nsfwass':
						try {
							...
							buffer = await getBuffer(res.url)
							client.sendMessage(from, buffer, image, {quoted: mek, caption: 'te gusta (｡>﹏<｡)?'})
						} catch (e) {
							console.log(`Error :`, color(e,'red'))
							reply('❌ *ERROR* ❌')
						}
					...
					case 'nsfwsidebobs':
						try {
							...
							buffer = await getBuffer(res.url)
							client.sendMessage(from, buffer, image, {quoted: mek, caption: '(￢‿￢ )'})
						} catch (e) {
							console.log(`Error :`, color(e,'red'))
							reply('❌ *ERROR* ❌')
						}
					...
					case 'nsfwahegao':
						try {
							...
							buffer = await getBuffer(res.url)
							client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Disfruta jsjs'})
						} catch (e) {
							console.log(`Error :`, color(e,'red'))
							reply('❌ *ERROR* ❌')
						}
					...
					case 'nsfwthighs':
						try {
							...
							buffer = await getBuffer(res.url)
							client.sendMessage(from, buffer, image, {quoted: mek, caption: 'ten muslos (>﹏<)'})
						} catch (e) {
							console.log(`Error :`, color(e,'red'))
							reply('❌ *ERROR* ❌')
						}
					...
					case 'nsfwfeets':
						try {
							...
							buffer = await getBuffer(res.url)
							client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Ten patas xd'})
						} catch (e) {
							console.log(`Error :`, color(e,'red'))
							reply('❌ *ERROR* ❌') 
						}
					...
					case 'nsfwarmpits':
						try {
							...
							buffer = await getBuffer(res.url)
							client.sendMessage(from, buffer, image, {quoted: mek, caption: 'mmm ok'})
						} catch (e) {
							console.log(`Error :`, color(e,'red'))
							reply('❌ *ERROR* ❌')
						}
						...
						try {
							...
							buffer = await getBuffer(res.url)
							client.sendMessage(from, buffer, image, {quoted: mek, caption: 'jsjsjs'})
						} catch (e) {
							console.log(`Error :`, color(e,'red'))
							reply('❌ *ERROR* ❌')
						}
				...
				if (!isUser) return reply(mess.only.daftarB)
			       ...
			       reply(anu.result.jadwalTV)
					break
                                case 'afk':
                                        tels = body.slice(4)
                                        if (args.length < 1) return reply('mmm ok?')
                                        if (!isUser) return reply(mess.only.daftarB)
				...
				case 'ttp':
					...
					break
                                case 'clearall':
					if (!isOwner) return reply('Quién eres?')
					anu = await client.chats.all()
